...
//
// Decodes JPEG, PNG and SVG sitting in PSRAM into a downscaled RGB565 bitmap. Every
...
/* Decode the image in PSRAM [base, base+len) into a freshly malloc'd RGB565 bitmap,
   downscaled (integer step) to fit within (max_w, max_h). On success returns 1 and sets
   *out (free() it), *ow, *oh. Returns 0 on unsupported format / decode error / when it
...
/* Format sniff for the magic bytes already in `sig` (>=8 bytes). 0 none / 'j' jpeg /
   'p' png / 's' svg. Lets the caller decide "is this response an image?" cheaply. */
