...
typedef int8_t xm_sample_point_t;
...
/** Returns the number of bytes required for the output buffer of
    xm_save_context(). */
uint32_t xm_save_size(const xm_context_t*)
__attribute__((warn_unused_result))
__attribute__((nonnull));

/** Save a loaded context to a memory buffer. This format can be loaded again
    with xm_create_context() and *is* portable across different libxm builds. */
void xm_save_context(const xm_context_t* restrict, char* restrict out)
__attribute__((nonnull));
...
/** Returns the number of bytes required for the output buffer of
    xm_dump_context(). */
uint32_t xm_dump_size(const xm_context_t*)
__attribute__((warn_unused_result))
__attribute__((nonnull));

/** Dump a loaded context to a memory buffer. This format is highly non-portable
 * and is meant for static linking only.
 *
 * If the context has been used to generate audio samples, its current playback
 * position (and effect memories, etc) will also be saved; use
 * xm_reset_context() before dumping to prevent this.
 *
 * @param ctx the context to dump
 *
 * @param out[.xm_dump_size()] a pool of allocated memory, at least
 * xm_dump_size(ctx) bytes long
 */
void xm_dump_context(xm_context_t* restrict ctx, char* restrict out)
__attribute__((nonnull));

/** Restore a context previously generated by xm_dump_context().
 *
 * This function will produce smaller code size compared to xm_create_context(),
 * but doesn't do any kind of error checking.
 *
 * @param data pointer to **writeable** data generated by xm_dump_context();
 ...
 * immediately after xm_create_context() or xm_reset_context(), with a value
 * that matches the system's audio output, or a standard value like 44100 or
 * 48000 if outputting to a file. Has no effect if libxm was compiled with a
 * hardcoded value (XM_SAMPLE_RATE).
 */
 ...
 *
 * @returns pointer to a string, or NULL on invalid inputs */
const char* xm_get_sample_name(const xm_context_t*, uint16_t)
__attribute__((warn_unused_result))
__attribute__((returns_nonnull))
__attribute__((nonnull));
...
/** Get the number of samples in the entire module. */
uint16_t xm_get_number_of_samples(const xm_context_t*)
__attribute__((warn_unused_result))
__attribute__((nonnull));
 ...
 */
xm_sample_point_t* xm_get_sample_waveform(xm_context_t*,
                                          uint16_t sample,
                                          uint32_t* out_length)
__attribute__((warn_unused_result))
__attribute__((returns_nonnull))
 ...
 * xm_get_number_of_instruments(...).
 *
 * @note Sample numbers go from 0 to xm_get_nubmer_of_samples(...)-1.
 */
uint32_t xm_get_latest_trigger_of_sample(const xm_context_t*, uint16_t)
__attribute__((warn_unused_result))
__attribute__((nonnull));
